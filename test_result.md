...
##---- 2026-07-12 · Bug fix: Sign in button on /?admin=1 gate did nothing ----

agent_communication:
    - agent: "main"
      message: |
        USER REPORTED BUG (screenshot attached in chat): Visiting https://hope-steps.preview.emergentagent.com/?admin=1 while not logged in shows the "Admin access required" card. Tapping the "Sign in" button did nothing — the Onboarding dialog never opened.

        ROOT CAUSE: In /app/app/page.js the App component returns EARLY for the admin-gated route (`if (adminRequested) { ... return <Card>...<Button onClick={()=>setOnboard(true)}>Sign in</Button>... }`). The <Onboarding open={onboard} .../> dialog component was only rendered inside the later landing and dashboard return branches, so the click sets `onboard=true` but there is no <Onboarding> in the JSX tree to actually pop up.

        FIX APPLIED in /app/app/page.js:
        - Rendered the <Onboarding> dialog inside the admin-gated return block as well:
          `<Onboarding open={onboard} onClose={()=>setOnboard(false)} onDone={(u)=>{setMe(u);setOnboard(false);setTimeout(()=>window.location.reload(),300)}}/>`
        - After sign-in on the admin-gated page, a hard reload is performed so the `/api/me` role fetch runs with a fresh cookie and the admin dashboard renders if role='admin'.
        - Added a small helper line: if user is signed in but role !== 'admin', we now display "Signed in as <name> but not an admin." beneath the buttons for clarity.

        Other fixes from same session (already in place, unchanged):
        - Persistent "Log Out" button in the dashboard header next to avatar (top-right, visible on mobile as icon-only + desktop with label).
        - Admin sidebar has TWO buttons: "Exit Admin" (removes ?admin=1) and "Log Out" (calls sb.auth.signOut()).
        - Email + password login: added 200ms wait for cookie sync + hard reload after successful auth to guarantee /api/me returns the correct session.
        - /api/me endpoint now fetches fresh app_metadata via `sb.auth.admin.getUserById(user.id)` and re-runs auto-promotion for ADMIN_EMAIL on every call — so freshly-promoted admin sees the role without needing to log out and back in.
        - /?signout=1 URL parameter force-clears any stuck Supabase session and localStorage.

        REQUESTED VERIFICATION FOR THIS BUG:
        1. GET https://hope-steps.preview.emergentagent.com/?admin=1 → should render "Admin access required" card with a purple gradient "Sign in" button when there's no session cookie.
        2. Clicking the "Sign in" button MUST open the Onboarding dialog with Google OAuth button + email/password form. Playwright: click the button, then wait for the dialog to appear (look for text "Join RoseUp Quest 2026" or the "Continue with Google" button visible).
        3. Verify the "Back to app" button navigates to `/`.
        4. Verify that when NOT authenticated, /?admin=1 does NOT render the admin dashboard (should show only the gate).

        Also please re-run these regression checks (they were passing before, should still pass):
        - GET /api/me returns { user: null } for unauthenticated requests (HTTP 200).
        - GET /api/stats HTTP 200 with Supabase data.
        - The OAuth URL generated by clicking "Continue with Google" on the landing page (from Sign Up dialog) still has `redirect_uri=https://nizhognfqlrivxwbesur.supabase.co/auth/v1/callback`, `prompt=select_account`, and NO localhost / :3000 / 0.0.0.0 leak.

        DO NOT complete a real Google OAuth login — just inspect the URL as before.
        DO NOT modify any code. Report only pass/fail per item with evidence.

test_plan:
  current_focus:
    - "Sign in button on /?admin=1 opens the Onboarding dialog"
    - "Admin-gated page shows 'Admin access required' when unauthenticated"
    - "Back to app button navigates to /"
    - "OAuth URL from Sign Up on landing still has correct redirect_uri, prompt=select_account, no localhost leak"
    - "GET /api/me returns { user: null } for unauth requests"
  stuck_tasks: []
  test_all: false
  test_priority: "high_first"

frontend:
  - task: "Sign in button on /?admin=1 opens the Onboarding dialog"
    implemented: true
    working: true
    file: "/app/app/page.js"
    stuck_count: 0
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Fixed by rendering <Onboarding> dialog inside the admin-gated return block (line 707). The dialog was previously only rendered in landing/dashboard branches, so clicking 'Sign in' set onboard=true but no dialog appeared in the JSX tree."
        - working: true
          agent: "testing"
          comment: "VERIFIED (Desktop + Mobile): Navigated to /?admin=1 after clearing session via /?signout=1. Admin gate card renders with RoseUp brand mark, heading 'Admin access required', description, purple gradient 'Sign in' button, and 'Back to app' button. Clicking 'Sign in' successfully opens the Onboarding dialog with all expected elements: RoseUp brand mark, heading 'Join RoseUp Quest 2026', 'Continue with Google' button with colored G logo, email + password inputs, submit button ('Log in'), and toggle link. Dialog closes with ESC key. 'Back to app' button navigates to /. Mobile viewport (375x812) also works correctly. Screenshots: admin-gate-card.png, onboarding-dialog-opened.png, mobile-onboarding-dialog.png. THE PRIMARY BUG IS FIXED."
  
  - task: "OAuth URL from Sign Up on landing has correct redirect_uri, prompt=select_account, no localhost leak"
    implemented: true
    working: true
    file: "/app/app/page.js"
    stuck_count: 0
    ...
    status_history:
        - working: true
          agent: "testing"
          comment: "VERIFIED: Captured OAuth URL from 'Continue with Google' button on landing page Sign Up dialog. URL: https://accounts.google.com/o/oauth2/v2/auth?access_type=offline&client_id=376731476531-0r5b3epa3tdhc1nnf817u4f5lhdsv2it.apps.googleusercontent.com&prompt=select_account&redirect_to=https%3A%2F%2Fhope-steps.preview.emergentagent.com%2Fauth%2Fcallback&redirect_uri=https%3A%2F%2Fnizhognfqlrivxwbesur.supabase.co%2Fauth%2Fv1%2Fcallback&response_type=code&scope=email+profile&state=... ✅ redirect_uri is EXACTLY correct (Supabase callback). ✅ prompt=select_account is present. ✅ redirect_to contains production URL (hope-steps.preview.emergentagent.com). ✅ NO localhost, :3000, :8000, :8080, 127.0.0.1, or 0.0.0.0 found in URL. The iPhone Google OAuth bug fix is still working correctly."
  
  - task: "Backend endpoints /api/me and /api/stats return correct data"
    implemented: true
    working: true
    file: "/app/app/api/[[...path]]/route.js"
    stuck_count: 0
    ...
    status_history:
        - working: true
          agent: "testing"
          comment: "VERIFIED: GET /api/me (unauthenticated) returns { user: null } with HTTP 200. GET /api/stats returns HTTP 200 with all required fields: totalPoints, totalKm, totalParticipants, totalDonations, fundGoal (250000), topParticipants. Backend is functioning correctly."
  
  - task: "Landing page renders without errors"
    implemented: true
    working: true
    file: "/app/app/page.js"
    stuck_count: 0
    ...
    status_history:
        - working: true
          agent: "testing"
          comment: "VERIFIED: Landing page renders correctly with header (RoseUp brand + nav links: Home, Leaderboard, Donations, How It Works), hero section ('RoseUp Quest 2026' heading + 'Every Step Gives Hope' tagline), stat cards row, blue rose SVG illustration, 'How It Works' 5-step section. No console errors detected. Full-page screenshot captured: landing-page-full.png."

agent_communication:
    - agent: "testing"
      message: |
        VERIFICATION COMPLETE (2026-08-18) - All tests PASSED.
        
        PRIMARY BUG FIX VERIFIED ✅:
        The user-reported bug where the "Sign in" button on /?admin=1 did nothing is now FIXED. The Onboarding dialog opens correctly on both desktop (1920x1080) and mobile (375x812) viewports when clicking the "Sign in" button. All dialog elements are present and functional.
        
        REGRESSION CHECKS PASSED ✅:
        - OAuth URL still uses production URL only (no localhost/internal port leaks)
        - prompt=select_account is present for account switching
        - Backend endpoints /api/me and /api/stats return correct data
        - Landing page renders without errors
        
        EVIDENCE:
        - admin-gate-card.png: Admin gate with Sign in button
        - onboarding-dialog-opened.png: Dialog opened after clicking Sign in (desktop)
        - mobile-onboarding-dialog.png: Dialog on mobile viewport
