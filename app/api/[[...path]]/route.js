...
async function handler(request, ctx) {
  ...
  try {
    ...
    if (method === 'GET' && path === 'me') {
      ...
      // Ensure a participant row exists (fallback if trigger missed)
    ...
    if (method === 'GET' && path === 'stats') {
      const [{ data: parts }, { count: participantsCount }, { data: top }, { data: meta }] = await Promise.all([
        ...
        sb.from('meta').select('value').eq('key', 'config').maybeSingle(),
      ])
      ...
      return NextResponse.json({
        ...
        totalDonations: Math.round(totalPoints * 1.25 + 12480),
        fundGoal,
    ...
    if (method === 'GET' && path === 'challenges/daily') {
      ...
      const { data } = await sb.from('challenges')
        .select('*').eq('type', 'daily').eq('active', true).order('points', { ascending: false })
      let done = new Set()
    ...
    if (method === 'GET' && path === 'challenges') {
      // Admin: return all daily challenges (active + inactive pool)
      const { data } = await sb.from('challenges').select('*').eq('type', 'daily').order('created_at', { ascending: false })
    ...
    if (method === 'POST' && path === 'challenges') {
      ...
      const { data } = await sb.from('challenges').insert({
        id, type: 'daily', title: b.title || 'Untitled', description: b.description || '',
        icon: b.icon || '⭐', points: Number(b.points) || 20, active: b.active !== false, category: b.category || null,
      }).select().single()
    ...
    if (method === 'PUT' && path.startsWith('challenges/')) {
      const id = path.split('/')[1]
      const b = await request.json()
      const { id: _i, _id, createdAt, created_at, type, ...upd } = b
      const { data } = await sb.from('challenges').update({ ...upd, type: 'daily' }).eq('id', id).select().single()
      return NextResponse.json(data)
    }
    ...
    if (method === 'POST' && path === 'challenges/complete') {
      ...
      await sb.from('challenge_completions').insert({ user_id: userId, challenge_id: challengeId }).catch(() => {})
    ...
    if (method === 'POST' && path === 'submissions') {
      ...
      // Legacy fallback: data URL upload (kept for small images only)
      ...
      const { data } = await sb.from('submissions').insert({
        ...
        challenge_id: b.challengeId, challenge_title: b.challengeTitle, challenge_type: b.challengeType || 'daily',
    ...
    // ---------- Manual daily rotation trigger (admin) ----------
    if (method === 'POST' && path === 'admin/rotate-daily') {
      const { error } = await sb.rpc('rotate_daily_challenges')
      if (error) return NextResponse.json({ error: error.message }, { status: 500 })
      return NextResponse.json({ ok: true })
    }
    ...
    if (method === 'GET' && path === 'admin/analytics') {
      const [{ data: participants }, { data: submissions }, { data: challenges }] = await Promise.all([
        ...
        sb.from('challenges').select('*').eq('type', 'daily'),
      ])
